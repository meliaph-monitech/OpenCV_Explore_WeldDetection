...
def smooth_1d(arr: np.ndarray, k: int) -> np.ndarray:
    ...
    if arr.size == 0:
        return arr
    pad = np.pad(arr.astype(np.float32), (k // 2, k // 2), mode="edge")
...
# 5) NEW: Sobel Band Edges (Upper/Lower seam) -----------------------------

def method_sobel_band_edges(bgr: np.ndarray,
                            gray: np.ndarray,
                            band_ratio: float,
                            blur_ksize: int,
                            smooth_win: int,
                            ...
                            alpha: float) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    - Take a central horizontal band (band_ratio of image height).
    - Compute vertical gradient magnitude (|Sobel Y|).
    - For each column, pick strongest edge above and below midline.
    - Smooth seams, enforce a minimum gap, and fill between seams.
    """
    H, W = gray.shape
    band_ratio = float(np.clip(band_ratio, 0.05, 1.0))
    band_h = int(max(4, round(H * band_ratio)))
    top = (H - band_h) // 2
    bot = top + band_h
    ...
    # Split into upper/lower halves and find strongest vertical edges per column
    rh, rw = mag.shape
    mid = rh // 2
    upper = mag[:mid, :]
    lower = mag[mid:, :]

    # Handle degenerate bands
    if upper.size == 0 or lower.size == 0:
        heatmap = apply_heatmap01(normalize01(mag))
        return heatmap, np.zeros((H, W), np.uint8), cv2.cvtColor(bgr, cv2.COLOR_BGR2RGB)

    upper_edges = np.argmax(upper, axis=0)           # 0..mid-1
    lower_edges = np.argmax(lower, axis=0) + mid     # mid..rh-1

    # Smooth the 1D seams
    upper_s = smooth_1d(upper_edges.astype(np.float32), smooth_win)
    lower_s = smooth_1d(lower_edges.astype(np.float32), smooth_win)

    # Enforce a minimum gap between upper and lower edges
    min_gap = int(max(1, round(min_gap_pct * rh)))
    valid = (lower_s - upper_s) > min_gap

    # Build mask by filling between the seams (only where valid)
    mask = np.zeros((H, W), np.uint8)
    for x in range(W):
        if valid[x]:
            y1 = int(np.clip(upper_s[x] + top, 0, H - 1))
            y2 = int(np.clip(lower_s[x] + top, 0, H - 1))
            if y2 > y1:
                mask[y1:y2, x] = 255

    # Make a BGR overlay with red/green seam markers
    overlay = bgr.copy()
    for x in range(0, W, max(1, W // 400 + 1)):  # sparse dots for speed
        y_up = int(np.clip(upper_s[x] + top, 0, H - 1))
        y_lo = int(np.clip(lower_s[x] + top, 0, H - 1))
        cv2.circle(overlay, (x, y_up), 1, (0, 0, 255), -1)  # red = upper
        cv2.circle(overlay, (x, y_lo), 1, (0, 255, 0), -1)  # green = lower

    overlay = overlay_mask_on_image(overlay, mask, alpha=alpha, color=(255, 0, 0))
...
TAB_TITLES = [
    ...
    "CLAHE + Otsu / Adaptive",
    "Bright-Band Guided",
    "Sobel Band Edges (Upper/Lower)"  # NEW
]
...
with tab5:
    ...
    c1, c2, c3, c4, c5 = st.columns(5)
    with c1:
        band_ratio = st.slider("Band height (fraction of H)", 0.05, 1.0, 0.35, step=0.01)
    with c2:
        blur_ksize = st.slider("Pre-blur ksize", 0, 21, 7, step=1)
    with c3:
        smooth_win = st.slider("Seam smoothing window", 3, 201, 61, step=2)
    with c4:
        min_gap_pct = st.slider("Min gap (% of band H)", 0.0, 0.5, 0.06, step=0.01)
    with c5:
        alpha = st.slider("Overlay alpha", 0.0, 1.0, 0.5, step=0.05, key="sobel_alpha")

    heatmap, binary, overlay = method_sobel_band_edges(
        bgr=bgr,
        gray=gray,
    ...
    cA, cB, cC = st.columns(3)
    with cA:
        st.caption("Vertical Gradient Heatmap (ROI band)")
        st.image(cv2.cvtColor(heatmap, cv2.COLOR_BGR2RGB), use_column_width=True)
    with cB:
        st.caption("Band-Filled Mask (between seams)")
        st.image(binary, use_column_width=True)
    with cC:
        st.caption("Overlay (upper:red, lower:green, mask:red fill)")
        st.image(cv2.cvtColor(overlay, cv2.COLOR_BGR2RGB), use_column_width=True)

    with st.expander("Download results"):
        download_png_button(heatmap, "Download Heatmap", key="sobel_heatmap")
        download_png_button(cv2.cvtColor(binary, cv2.COLOR_GRAY2BGR), "Download Binary", key="sobel_binary")
        download_png_button(overlay, "Download Overlay", key="sobel_overlay")
